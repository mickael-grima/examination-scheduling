...
def build_model(data, n_cliques = 0, verbose = True):
    ...
    # for i in range(2,n):
    #     for k in range(i,r):
    #         model.addConstr( quicksum(x[i,k,l] for l in range(p) if T[k][l] == 1) <= quicksum( quicksum(x[i2,k-1,l] for l in range(p)if T[k-1][l] == 1 ) for i2 in range(i-1)  ) )
    ...
    #         for k in range(i,r):
    #             if T[k][l] == 1:
    #                 for k2 in reversed(range(k)):
    #                     if T[k2][l] == 1:
    #                         model.addConstr( x[i,k,l]  <= quicksum( x[i2,k2,l]  for i2 in range(i-1) ) )
    #                         break
    ...
    for l in range(p):
        for i in range(1,n):
            for k in range(i,r):
                if T[k][l] == 1:
                    model.addConstr(x[i,k,l] <= quicksum(x[i-1,k2,l] for k2 in range(k-1) if T[k2][l] == 1 ))
